"""CUDA-only exploratory full-set evaluator for existing, hash-verified images. One model per run."""
import argparse
import hashlib
...
def main():
    ...
    p.add_argument('--checks',type=Path,default=Path('data/semantic_eval_v3/checks.json'))
    p.add_argument('--tool',choices=['grounding_dino','qwen_vlm'],required=True)
    p.add_argument('--split',choices=['full','smoke'],default='full')
    ...
    rows = [r for r in json.loads(a.sample.read_text())['rows'] if r['split']==a.split and checks[r['input_id']]['primary_evaluator'].startswith(a.tool)]
    ...
    manifest = {'png_sha256':png_hashes,'generation_runs_sha256':hashlib.sha256((a.images/'runs.jsonl').read_bytes()).hexdigest(),'config':config,'tool':a.tool,'scope':'primary_only','split':a.split,'limit':a.limit,
        ...
        try:
            ...
            with torch.inference_mode():
                if a.tool=='qwen_vlm':
                    ...
                    raw=[]; retained=[]
                    for query_index, query in enumerate(e['detector_queries']):
                        ...
                        aliases=e.get('detector_label_aliases',[[x] for x in e['detector_queries']])[query_index]
                        accepted={normalize(x) for x in aliases}
                        valid=[j for j,label in enumerate(detected['text_labels']) if normalize(label) in accepted]
    ...
    print(f'Complete: {len(outcomes)} predictions, {errors} evaluation errors. Exploratory scores; not validated accuracy.')
